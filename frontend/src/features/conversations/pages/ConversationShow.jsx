...
export function ConversationShow() {
    ...
    const { data: chatMessages, isLoading: messagesIsLoading, isError: messagesError, fetchNextPage, hasNextPage, isFetchingNextPage } = useChatMessages(id);

    //SOCKET CONNECTION TO THE CHAT PRIVATE CHANNEL
    useChatSocket(id);
    ...
    return (
        <main className="flex flex-row w-full h-[calc(100vh-64px)] p-0 lg:h-full lg:p-2 lg:rounded-xl">
            ...
            {/* -- CURRENT CHAT -- */}
            <PageAnimWrapper uniqueKey={id} className="flex flex-col w-full h-full overflow-hidden justify-between backdrop-blur-sm backdrop-brightness-80 lg:w-[70%] lg:rounded-xl lg:pl-2">
