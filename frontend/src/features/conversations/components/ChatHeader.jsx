...
export function ChatHeader({ chat }) {
    ...
    return (
        <div id="chat-header" className="flex justify-between bg-[#171717] gap-4 items-center p-2 lg:p-3 lg:rounded-t-xl lg:gap-2">
            {/* -- PARTICIPANT LIST -- */}
            <ParticipantsModal participants={participants} enabled={participantsMenu} setParticipantsMenu={setParticipantsMenu}/>
            ...
            {isGroup && (
                <p onClick={() => setParticipantsMenu(!participantsMenu)} className="hover:cursor-pointer">Participants</p>
            )}
        </div>
    );
}
